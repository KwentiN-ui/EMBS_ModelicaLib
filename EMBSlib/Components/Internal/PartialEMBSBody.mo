...
partial model PartialEMBSBody
  parameter String SIDfileName="modelica://EMBSlib/Resources/Data/Beam_3m.SID_FEM" "Path to SID file";
  ...
  Real hd_t[3]=sum(identity(nr0)*nodes[i].f for i in 1:numNodes);
  ...
  Real hd_r[3]=sum(identity(nr0)*nodes[i].t for i in 1:numNodes);
  ...
  protected
   parameter String actualFileName=Modelica.Utilities.Files.loadResource(SIDfileName);
   parameter Integer numNodes = EMBSlib.Internal.ExternalFunctions.getNumNodesFromFile(actualFileName) annotation(Evaluate=true);
   parameter Integer numModes = EMBSlib.Internal.ExternalFunctions.getNumModesFromFile(actualFileName) annotation(Evaluate=true);
   parameter Integer nq=numModes;

   parameter EMBSlib.Internal.SID_File sid=EMBSlib.Internal.SID_File(actualFileName) annotation(Evaluate=true);
